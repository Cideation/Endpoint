...
from .api_models import (
    ...
    BulkOperationResponse, BulkOperationResult, ExportRequest, ExportResponse,
...
class FrontendAPIService:
    ...
    async def get_file_status(self, file_id: UUID) -> FileStatusResponse:
        ...
        try:
            ...
            query = """
                SELECT * FROM parsed_files WHERE file_id = $1
            """
            
            result = await self.db_manager.execute_query(query, (file_id,))
            ...
            if row['parsing_status'] == 'success':
                progress = 100
            elif row['parsing_status'] == 'processing':
                progress = 50  # Estimate
            elif row['parsing_status'] == 'error':
                progress = 0
    ...
    async def get_files(self, search_request: FileSearchRequest) -> FileListResponse:
        """Get paginated list of processed files"""
        try:
            await self.initialize()
            
            # Build query with filters
            query, params = self._build_file_search_query(search_request)
            ...
            # Add pagination
            offset = (search_request.page - 1) * search_request.page_size
            paginated_query = f"{query} ORDER BY pf.created_at DESC LIMIT {search_request.page_size} OFFSET {offset}"
            
            # Execute query
            results = await self.db_manager.execute_query(paginated_query, params)
            
            # Transform to FileProcessingStatus
            files = []
            for row in results:
                progress = 0
                if row['parsing_status'] == 'success':
    ...
    async def bulk_operation(self, request: BulkOperationRequest) -> BulkOperationResponse:
        ...
        try:
            ...
            if request.operation == "delete":
                for component_id in request.component_ids:
                    try:
                        await self._delete_component(component_id)
                        success_count += 1
                    except Exception as e:
                        error_count += 1
                        errors.append({
                ...
                new_type = request.parameters.get("component_type")
                if new_type:
                    for component_id in request.component_ids:
                        try:
                            await self._update_component_type(component_id, new_type)
                            success_count += 1
                        except Exception as e:
                            error_count += 1
                            errors.append({
    ...
    def _build_file_search_query(self, search_request: FileSearchRequest) -> tuple:
        """Build SQL query for file search with filters"""
        base_query = "SELECT pf.* FROM parsed_files pf"
        ...
        if search_request.query:
            param_count += 1
            conditions.append(f"pf.file_name ILIKE ${param_count}")
            params.append(f"%{search_request.query}%")
        
        if search_request.file_type:
            param_count += 1
            conditions.append(f"pf.file_type = ${param_count}")
            params.append(search_request.file_type.value)
        
        if search_request.status:
            param_count += 1
            conditions.append(f"pf.parsing_status = ${param_count}")
            params.append(search_request.status.value)
        
        if search_request.uploaded_after:
            param_count += 1
            conditions.append(f"pf.created_at >= ${param_count}")
            params.append(search_request.uploaded_after)
        
        if search_request.uploaded_before:
            param_count += 1
            conditions.append(f"pf.created_at <= ${param_count}")
            params.append(search_request.uploaded_before)
    ...
    async def _delete_component(self, component_id: UUID):
        """Delete a component and all related data"""
        # Delete in order to respect foreign key constraints
        await self.db_manager.execute_command(
            "DELETE FROM component_materials WHERE component_id = $1", (component_id,)
        )
        await self.db_manager.execute_command(
            "DELETE FROM component_properties WHERE component_id = $1", (component_id,)
        )
        await self.db_manager.execute_command(
            "DELETE FROM parsed_components WHERE component_id = $1", (component_id,)
        )
        await self.db_manager.execute_command(
            "DELETE FROM spatial_data WHERE component_id = $1", (component_id,)
        )
        await self.db_manager.execute_command(
            "DELETE FROM dimensions WHERE component_id = $1", (component_id,)
        )
        await self.db_manager.execute_command(
            "DELETE FROM geometry_properties WHERE component_id = $1", (component_id,)
        )
        await self.db_manager.execute_command(
            "DELETE FROM components WHERE component_id = $1", (component_id,)
        )
    
    async def _update_component_type(self, component_id: UUID, new_type: str):
        """Update component type"""
        await self.db_manager.execute_command(
            "UPDATE components SET component_type = $1, updated_at = NOW() WHERE component_id = $2",
            (new_type, component_id)
        )
